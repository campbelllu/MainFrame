##this idea was good, but the data is absolute garbage. the abstraction layer on top of rankings is an absolute must
...
#         else:
#             avg = 0 #arbitraily chosen to give a middling score due to non-reporting
...
#             finalrating = 0
#         else:
#             finalrating = max(reqavg, ceqavg) #max(rfinalrating, cfinalrating)
#     except Exception as err:
...
#         #     finalrating = max(resultsdf['navgravg'][0], ceqavg, reqavg)
...
#         finalrating = max(repdivs, calcdivs)
...
#         if fcfavg == poravg == 0:
#             finalrating = 0
#         elif fcfavg == 0 and poravg != 0:
#             finalrating = 100 - poravg
#         elif poravg == 0 and fcfavg != 0:
#             finalrating = 100 - fcfavg
#         else:
#             finalrating = 100 - min(fcfavg, poravg)
...
# # print(payout_rating('ARCC'))
# # print(payout_rating('TXN'))
# # print(payout_rating('O'))
# # print(payout_rating('MSFT'))
...
# # print(reitroce_rating('ADC'))
# # print(reitroce_rating('O'))
...
#                 srmaxscore = 1000 #5*((revv) + (niv) + (ffov) + (fcfv) + (fcfmv) + (debtv) + (equityv) + (bvv) + (cfv) + (sharesv) + (divpayv) + 
...
#                 uploadToDB(uploaddf,'Sector_Rankings2')
#                 n += 1
#             except Exception as err:
...
#     conn = sql.connect(db_path)
#     query = conn.cursor()
#     sector = 'DELETE FROM Sector_Rankings2'
#     query.execute(sector)
#     conn.commit()
#     query.close()
#     conn.close()

#     print_DB('SELECT * FROM Sector_Rankings2','print')
...
# # sr1 = 'select * from Sector_Rankings WHERE Sector LIKE \'RE\' ORDER BY scorerank DESC' #WHERE Sector LIKE \'U\'
# # sr2 = 'select Ticker, reitroce, roce, ffopo, po, divgr, shares, bv, equity, cf, fcfm, ffo, ni, rev, maxscore, score, scorerank from Sector_Rankings2 WHERE Sector LIKE \'RE\' ORDER BY scorerank DESC'
# # print(print_DB(sr1,'return').head(50))
# # print(print_DB(sr2,'return').head(50))
